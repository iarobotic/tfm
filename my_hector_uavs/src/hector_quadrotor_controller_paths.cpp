...
Also, both the desired drone trajectory and the drone trajectory are published in "/tr_path" and "/path" topics, respectively, for visualizing in Rviz.
In "velocity_controller" function both the desired trajectory is generated and the signals control are computed.
For this example, the desired trajectory is a lemniscate.
First, the "/enable_motors" service is called for moving the simulated quadrotor. Take off and land functions are defined.
...
$ roslaunch my_hector_uavs visualization_paths.launch
	# Run Rviz with my configuration file to visualize the drone and the paths (optional)
*/
#include "ros/ros.h"
...
#include "nav_msgs/Path.h" //To visualize paths in Rviz
...
geometry_msgs::PoseStamped drone_pose, tr_pose; //Create global objects to define drone position and desired trajectory using geometry_msgs/Twist messages type

//Define objects to publish drone trajectory and desired trajectory in Rviz
ros::Publisher drone_path_pub, tr_path_pub;
nav_msgs::Path drone_path_msg, tr_path_msg; //Paths objects of nav_msgs/Path messages type
...
double t, t0, Vxy_max = 1, Vz_max = 1.0, Wz_max = 4; ///Timer, initial time, maximum translational velocities [m/s] and the maximum rotational velocities [rad/s]
...
void poseCallback(const nav_msgs::Odometry::ConstPtr msg){ //Callback function to obtain the posture of the drone
  ...
  drone_pose.pose.position = msg->pose.pose.position; //Copy the drone position to the "drone_pose" object
  drone_path_msg.poses.push_back(drone_pose); //Add the new position to the list
}
...
void velocity_controller(){ //Function to generate the desired trajectory and to compute the signals control
  ...
  //Assign the desired position to the "tr_pose" object
  tr_pose.pose.position.x = Xd; tr_pose.pose.position.y = Yd; tr_pose.pose.position.z = Zd;
  tr_path_msg.poses.push_back(tr_pose); //Add the new point to the list
...
{
  ros::init(argc, argv, "hector_quadrotor_controller_paths");  //Initialize the node
  ...
  do{
    ...
    if(key == 'c'){ //Clean up both paths
      drone_path_msg.poses.clear(); tr_path_msg.poses.clear(); //Clear paths messages
