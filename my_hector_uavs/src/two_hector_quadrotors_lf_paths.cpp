/* 
@description:
With this node, both the desired leader drone trajectory and the trajectory of each drone are published in the corresponding topics for visualizing in Rviz.
...
The "/enable_motors" service of each drone is called. Take off and land functions are defined.
...
$ roslaunch my_hector_uavs two_visualization_paths.launch
	# Run Rviz with my configuration file to visualize two drones and their paths (optional)
*/
#include "ros/ros.h"
...
geometry_msgs::PoseStamped drone_pose1, drone_pose2, tr_pose; //Create global objects to define drones' position and desired trajectory using geometry_msgs/Twist messages type
...
void poseCallback1(const nav_msgs::Odometry::ConstPtr msg){ //Callback function to obtain the posture of the drone 1
  ...
  drone_pose1.pose.position = msg->pose.pose.position; //Copy the drone position to the "drone_pose1" object
  drone_path_msg1.poses.push_back(drone_pose1); //Add the new position to the list
...
void poseCallback2(const nav_msgs::Odometry::ConstPtr msg){ //Callback function to obtain the posture of the drone 2
  ...
  drone_pose2.pose.position = msg->pose.pose.position; //Copy the drone position to the "drone_pose2" object
  drone_path_msg2.poses.push_back(drone_pose2); //Add the new position to the list
...
void follower_vel_control(){ //Function to compute the signals control of the follower robot
  ...
  //Translational velocities of the leader drone in the glabal frame
  xlp = Vxl*cos(r_pose1.yaw)-Vyl*sin(r_pose1.yaw);
  ylp = Vxl*sin(r_pose1.yaw)+Vyl*cos(r_pose1.yaw);

  //Corresponding desird time derivatives
...
{
  ros::init(argc, argv, "two_hector_quadrotors_lf_paths");  //Initialize the node
  ...
  do{
    ...
    if(key == 'c'){ //Clean up both paths
      drone_path_msg1.poses.clear(); drone_path_msg2.poses.clear(); tr_path_msg.poses.clear(); //Clear paths messages
      printf(" Clear paths\n");
      key = 0; //Reset the key
    }
